...
def _is_percentage_question(question: AnalysisQuestion) -> bool:
    return "percent" in question.key or "percentage" in question.question.lower()


def _format_scalar_answer(question: AnalysisQuestion, value: Any) -> str:
    if value is None:
        return "No result"
...
def format_result(question: AnalysisQuestion, row: Optional[Dict[str, Any]], rows: List[Dict[str, Any]]) -> str:
    ...
    if not row:
        return "No result"
    return _format_scalar_answer(question, row.get("answer"))
...
def expected_result_keys() -> set[str]:
    """Return the keys every analysis result dictionary exposes to templates."""

    return {"key", "question", "sql", "explanation", "rows", "answer"}


def empty_analysis_results() -> List[Dict[str, Any]]:
    """Return a deterministic fallback result so the page still renders an Answer label."""

    return [
        {
            "key": "database_unavailable",
            "question": "Analysis unavailable until PostgreSQL is configured.",
            "sql": "",
            "explanation": "The Flask page rendered without live database results.",
            "rows": [],
            "answer": "No result",
        }
    ]
...
def main() -> None:  # pragma: no cover - exercised manually from the command line.
